...
import 'utils.dart'
    show snackBarAlert, Config, DecimalTextInputFormatter, removeTailSlash;
...
class ConfigPage extends StatefulWidget {
  ...
  const ConfigPage(
      {super.key, required this.updateFunc, required this.currentConfig});
...
class ConfigPageState extends State<ConfigPage> {
  ...
  bool isLongingAlgorithmEnabled = false;
    ...
    storage.getApiConfigs().then((List<Config> value) {
      setState(() {
        ...
        frequencyPenaltyController.text =
            widget.currentConfig.frequencyPenalty ?? "";
        presencePenaltyController.text =
            widget.currentConfig.presencePenalty ?? "";
        maxTokensController.text = widget.currentConfig.maxTokens ?? "";
      });
    });

    // 读取思念算法状态
    storage.getLongingAlgorithmStatus().then((bool value) {
      setState(() {
        isLongingAlgorithmEnabled = value;
      });
    });
  }
            ...
            child: Column(
              children: [
                Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                  ...
                  IconButton(
                      onPressed: qrShare, icon: const Icon(Icons.qr_code)),
                ]),
                ...
                const SizedBox(height: 20),
                SwitchListTile(
                  title: const Text('接受角色回复'),
                  secondary: const Icon(Icons.favorite_border),
                  value: isLongingAlgorithmEnabled,
                  onChanged: (bool value) {
                    ...
                    if (value) {
                      debugPrint("思念算法已开启");
                      LongingAlgorithmService.start();
                    } else {
                      debugPrint("思念算法已关闭");
                      LongingAlgorithmService.stop();
                    }
                    storage.setLongingAlgorithmStatus(value);
                  },
                ),
                const SizedBox(height: 20),
                Row(
                      ...
                      onPressed: () {
                        if (nameController.text.isEmpty ||
                          ...
                          Config newConfig = Config(
                            ...
                            frequencyPenalty:
                                frequencyPenaltyController.text.isEmpty
                                    ? null
                                    : frequencyPenaltyController.text,
                            presencePenalty:
                                presencePenaltyController.text.isEmpty
                                    ? null
                                    : presencePenaltyController.text,
                ...
                const Center(
                    child: Opacity(opacity: 0.3, child: Text("@k96e"))),
              ],
            ),
          )),
    );
